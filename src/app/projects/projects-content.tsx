...
function ProjectCardSkeleton({ aspect }: { aspect: 'large' | 'small' }) {
  const aspectClass = aspect === 'large' ? 'aspect-[4/3]' : 'aspect-[4/3]';
  return (
    <div className="border-input bg-card flex h-full flex-col gap-6 rounded-xl border py-6">
      <div className="px-6">
        <div
          className={cn(
            'bg-muted/40 w-full animate-pulse rounded-lg',
            aspectClass,
          )}
        />
      </div>
      <div className="space-y-3 px-6">
        <div className="bg-muted/40 h-4 w-32 animate-pulse rounded" />
        <div className="bg-muted/40 h-6 w-3/4 animate-pulse rounded" />
        ...
        <div className="flex flex-wrap gap-2 pt-2">
          <div className="bg-muted/40 h-5 w-16 animate-pulse rounded" />
          <div className="bg-muted/40 h-5 w-20 animate-pulse rounded" />
          <div className="bg-muted/40 h-5 w-14 animate-pulse rounded" />
        </div>
      </div>
    </div>
  );
      ...
      <section className="section-padding relative z-10">
        <div className="container">
          ...
          <div
            ...
            aria-hidden
          >
            {Array.from({ length: PROJECT_TAGS.length + 1 }).map((_, i) => (
              <div
                key={`filter-skeleton-${i}`}
                className="bg-muted/40 h-9 w-24 animate-pulse rounded-full"
              />
            ))}
          </div>
...
function ProjectsPageContent() {
  ...
  const setSelectedTag = useCallback(
    (tag: string | null) => {
      ...
      push(`/projects?${params.toString()}`, { scroll: false });
    },
    [searchParams, push],
  );
